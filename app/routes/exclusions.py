"""
API route for the exclusions screen.
"""
...
from app.services.exclusions_service import get_exclusions_service
...
router = APIRouter(prefix="/exclusions", tags=["analysis"])
...
async def get_exclusions(
    ...
    divisions, instructions = get_saved_exclusion_inputs(
        db=db,
    ...
    return build_exclusions_response(payload)


def get_saved_exclusion_inputs(
    db: Session,
    user_id: str,
    project_id: str,
) -> tuple[List[str], str]:
...
def run_exclusions_analysis(
    ...
    try:
        service = get_exclusions_service()

        return service.extract_exclusions(
            user_id=user_id,
        ...
        logger.error(f"Error generating exclusions: {str(exc)}")

        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error generating exclusions: {str(exc)}",
        ) from exc


def build_exclusions_response(
    payload: dict[str, Any],
) -> ExclusionsResponse:
    items = [
        ExclusionItem.model_validate(item)
    ...
    return ExclusionsResponse(
        ...
        filters=build_exclusion_filters(),
        items=items,
    )


def build_exclusion_filters() -> List[ExclusionFilter]:
    return [
        ExclusionFilter(
            code="all",
